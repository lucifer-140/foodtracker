<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
            <div class="mb-4 sm:mb-0">
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Browse all users to grow your network.</p>
            </div>
            <a href="{{ route('friends.index') }}" class="inline-flex items-center px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md font-semibold text-xs text-gray-700 dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 disabled:opacity-25 transition ease-in-out duration-150 shrink-0">
                My Friends
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if (session('success'))
                <div class="mb-6 p-4 bg-green-100 dark:bg-green-900/20 border border-green-400 text-green-700 dark:text-green-300 rounded-lg shadow" role="alert">
                    {{ session('success') }}
                </div>
            @elseif (session('error'))
                <div class="mb-6 p-4 bg-red-100 dark:bg-red-900/20 border border-red-400 text-red-700 dark:text-red-300 rounded-lg shadow" role="alert">
                    {{ session('error') }}
                </div>
            @endif
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                @forelse ($users as $user)
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg flex flex-col text-center transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-2xl">
                        <div class="h-32 bg-gray-100 dark:bg-gray-700 rounded-t-xl flex items-center justify-center">
                        ...
                        <div class="p-5 flex-grow flex flex-col justify-between">
                            <div>
                                <h4 class="font-bold text-xl text-gray-800 dark:text-gray-100 truncate">{{ $user->name }}</h4>
                ...
                @empty
                    <div class="col-span-full bg-white dark:bg-gray-800 rounded-lg shadow-md text-center py-16">
                        <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-gray-100">No Other Users Found</h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Looks like you're the first one here!</p>
                    </div>
                @endforelse
            </div>
