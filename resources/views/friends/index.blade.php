    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
            <div class="mb-4 sm:mb-0">
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Manage your friend requests and connections.</p>
            </div>
            <a href="{{ route('users.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150 shrink-0">
                Find New Friends
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            @if($pendingRequests->isNotEmpty())
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                    <h3 class="text-xl font-bold text-gray-800 dark:text-gray-100">Pending Requests</h3>
                    <div class="mt-4 space-y-4">
                        @foreach ($pendingRequests as $requester)
                            <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                                <div>
                                    <span class="font-medium text-gray-800 dark:text-gray-200">{{ $requester->name }}</span>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    @forelse ($friends as $friend)
                        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg flex flex-col text-center transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-2xl">
                            ...
                            <div class="p-5 flex-grow flex flex-col justify-between">
                                <div>
                                    <h4 class="font-bold text-xl text-gray-800 dark:text-gray-100 truncate">{{ $friend->name }}</h4>
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Friends since {{ $friend->pivot->updated_at->format('M Y') }}</p>
                                </div>
                                <div class="mt-6">
                    ...
                    @empty
                        <div class="col-span-full bg-white dark:bg-gray-800 rounded-lg shadow-md text-center py-16">
                            ...
                            <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-gray-100">No Friends Yet</h3>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Use the "Find New Friends" button to start connecting.</p>
                        </div>
                    @endforelse
                </div>
            </div>
